...
def takeCommand():
    ...
    try:
        ...
        print(ep)
        px.speak("I didn't understand, say that again please")
...
def greetings():
    ...
    if hour >= 6 and hour < 12:
        ...
        px.speak("Good Night")
    px.speak("I'm Mark2 here...")
    px.speak("How are you, sir...")
...
def hadoop():
    ...
    def hadoop_master():
        ...
        # sending software that requires for configuring hadoop setup
        ...
        # formatting directory and starting the service
    ...
    def hadoop_worker():
        ...
        for i in range(0, no_slaves):
            ...
            print("\nHadoop Configuration Done For Worker Node!!\n")
            px.speak("Hadoop Configuration Done For Worker Node!!")
            px.speak("And connected successfully to master!!")
    ...
    def hadoop_client():
        ...
        print("\nHadoop Configuration Done For Client!!\n")
        px.speak("Hadoop Configuration Done For Client!!")
...
def aws():
    ...
    def ec2_service():
        while True:
            ...
            print("\t\t\t5. Attach Volume\n\t\t\t6. Privious Menu\n\t\t\t7. EXIT")
            px.speak("What You Want To Do With EC2 Service")
            px.speak("Create Key Pair")
            px.speak("Create Security Group")
            ...
            if (("create" in query) and ("key" in query)):
                print("\n")

                px.speak("Enter Key Name")
                key_name = input("Enter Key Name : ")

                os.system("aws ec2 create-key-pair --key-name {} --query 'KeyMaterial' --output text > {}.pem".format(
                    key_name, key_name))

                px.speak("Key Pair Created Successfully")
                print("\nKey Pair Created Successfully")

            elif (("create" in query) and ("security" in query)):

                px.speak("Enter Security Group Name")
                security_grp = input("\nEnter Security Group Name : ")

                px.speak("Enter Security Group Description")
                security_grp_desc = input("\nEnter Security Group Description : ")

                oss.system("aws ec2 create-security-group --group-name {} --description '{}'".format(security_grp,
                                                                                                     security_grp_desc))

                px.speak("Security Group Create Successfully")
                print("\nSecurity Group Create Successfully")


            elif (("launch" in query) and ("instance" in query)):
                print("\n")
                ...
                # Launching instance
                os.system(
                    "aws ec2 run-instances --image-id {} --count {} --instance-type {} --key-name {} --security-group-ids {} --subnet-id {}".format(
                        image_id, count, instance_type, key_name, security_grp, subnet_id))
            ...
            # Creating volume
                ...
                os.system(
                    "aws ec2 create-volume --volume-type {} --size {} --availability-zone {}".format(voluem_type, size,
                                                                                                     avl_zone))
            ...
            # Attach volume to existing instance
                ...
                os.system(
                    "aws ec2 attach-volume --volume-id {} --instance-id {} --device {}".format(volume_id, instance_id,
                                                                                               device_name))
    ...
    def s3():
        ...
        while True:
            ...
            if (("create" in query) and ("bucket" in query)):
                ...
                os.system(
                    'aws s3api put-public-access-block  --bucket {} --public-access-block-configuration "BlockPublicAcls=false,IgnorePublicAcls=false,BlockPublicPolicy=false,RestrictPublicBuckets=false"'.format(
                        bucket_name))
        ...
        while True:
            ...
            if (("create" in query) and ("distribution" in query)):
                ...
                if ("yes" in choice):
                    ...
                    os.system("aws cloudfront create-distribution    --origin-domain-name {}.s3.amazonaws.com".format(
                        bucket_name))
...
def docker():
    ...
    while True:
        ...
        if (("install" in query) and ("docker" in query)):
            ...
            file_1 = open("docker.repo", "w")
            file_1.write('[docker]\n')
            file_1.write('baseurl=https://download.docker.com/linux/centos/7/x86_64/stable/\n')
            file_1.write('gpgcheck=0')
            file_1.close()
            px.speak("Template created for docker repo")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        elif (("aws" in query) or ("a w s" in query)):
